...
class MysqlHelper:
    ...
    def findOneWithoutClose(self, sql, params=[]):
        # 一次只返回一行查询到的数据,不关闭连接
        try:
            cs1 = self.conn.cursor()
            cs1.execute(sql, params)
    ...
    def findAllWithoutClose(self, sql, params):
        # 接收全部的返回结果行
        # 返回查询到的全部结果值
        try:
            cs1 = self.conn.cursor()
    ...
    def cudWithoutClose(self, sql, params=[]):
        try:
            # 用来获得python执行Mysql命令的方法
            cs1 = self.conn.cursor()
            # 真正执行MySQL语句
...
def xgmmfun(Entry1,Entry2,Entry3,windows4):  #修改密码方法
    ...
    conn = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn1 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn2 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    ...
    if result == 0:
        ...
        if result1[0] != superpw:
            ...
            if len(newpw)<5:
                ...
                res2 = conn2.fetchone(sql4,xuser)
                if res2[0] == upwd2 :
                    windows4.title('新密码和原密码相同!!')
                else:
                    conn1.conn.cursor().execute(sql3,(upwd2,xuser))
                    conn1.conn.commit()
                    conn1.conn.close()
...
def get():   #登陆函数
    ...
    conn = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn1 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn5 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn6 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    ...
    result1 =conn1.fetchone(sql1,params)
    result2 =conn5.fetchone(sql3,usname)
    result3 = conn6.conn.cursor().execute(sql4,params)
    ...
    if result3 == 0 :
        ...
        conn3 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
        id1=conn3.fetchone(sql3,usname)
        if id1[0] == None:   #第一次登陆成功把机器码存进数据库
            id=get_cpu()
            conn4 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
            conn4.conn.cursor().execute(sql2,(id,usname))
            conn4.conn.commit()
            print('success')
        else:
...
def daoqi(): #调用到期窗口
    ...
    windows6 = tk.Tk()
    windows6.title('警告')
    windows6.geometry('130x90')
    windows.resizable(0,0)
    Label1=tk.Label(windows6,text='该账户已经到期了!!')
    Label1.place(height = 27,width = 131,x = 5,y = 9)
    Button1=tk.Button(windows6,text='确定',command= lambda: clos())
    Button1.place(height = 25,width = 68,x = 30,y = 45)
    tk.mainloop()
...
def  zhuanzhangfc(Entry1,Entry2):  #转账方法
    ...
    conn = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    myconn = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8').conn
    myconn1 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    myconn2 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
...
def registFun(e1, e2, e3,u1,sp, windows1):    #注册函数
    ...
    conn1 = MysqlHelper(host='192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8').conn
    sql2 = 'select recode from code where recode = %s '
    code1=conn1.cursor().execute(sql2,recode)
    conn1.commit()
    if code1 == 1:
        conn = MysqlHelper(host='192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8').conn
        cus1 = conn.cursor()
        ...
        if s == 0 and regpass == repassagin and len(regpass)>6 and len(superpass)>6 :
            ...
            conn.commit()
            conn.close()
            conn2 = MysqlHelper(host='192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8').conn
            conn2.cursor().execute(sql3,(0,reguser))
            conn2.cursor().execute(sql5,(ti,reguser))
...
def up(user1,num1,num2,num3,person,windows1):   #充值函数判断
    conn_x = MysqlHelper('192.168.0.105', port=3306,  db='test', user='root', passwd='123456', charset='utf8')
    conn3 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn4 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn5 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn6 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn7 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn8 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn9 = MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8')
    conn10 = MysqlHelper('192.168.0.105', port=3306,db='test', user='root', passwd='123456', charset='utf8')
    ...
    nu3=num3.get()  #卡3
    pers=person.get()  #推广人
    RES = conn10.fetchone(sql1,user)
    res=conn_x.conn.cursor().execute(sql1,user)
    isuser=conn_x.conn.cursor().execute(sql1,pers)   #查询推广人是否存在
    resisuser=conn_x.findOneWithoutClose(sql1,pers)   #查询推广人
    isovertime=conn_x.findOneWithoutClose(sql3,pers)
    conn_x.closeConn()
    ...
    res1=conn3.conn.cursor().execute(sql2,nu1)       #返回值为1证明该注册码为真
    res2=conn3.conn.cursor().execute(sql2,nu2)       #返回值为1证明该注册码为真
    res3=conn3.conn.cursor().execute(sql2,nu3)       #返回值为1证明该注册码为真
    result = conn3.fetchone(sql3,user)   #用户剩余时间
    if res1+res2+res3 == 3 :  #全部正确
        if len(pers) == 0:  #推广人为空
            t1=result[0]+datetime.timedelta(days=31)   #增加后的时间
            conn4.conn.cursor().execute(sql4,(t1,user))
            conn4.conn.commit()
            conn4.conn.close()
            windows1.title('充值成功，当前账户加时31天!!')
            # conn5.conn.cursor().execute(sql5,nu1)
            # conn5.conn.cursor().execute(sql5,nu2)  #注册成功删除注册卡
            # conn5.conn.cursor().execute(sql5,nu3)
            MysqlHelper('192.168.0.105', port=3306, db='test', user='root', passwd='123456', charset='utf8').delete(r'delete from registercode where paycode in (%s,%s,%s)', [str(nu1), str(nu2), str(nu3)])
            ...
            conn5.conn.cursor().execute(sql4,(t1,user))
            conn5.conn.cursor().execute(sql4,(t2,pers))
            conn5.conn.commit()
            conn5.conn.close()
        ...
        if len(pers) == 0:  #推广人为空
            ...
            conn6.conn.cursor().execute(sql4,(t1,user))
            conn6.conn.commit()
            conn6.conn.close()
            ...
            conn7.conn.cursor().execute(sql4,(t1,user))
            conn7.conn.cursor().execute(sql4,(t2,pers))
            conn7.conn.commit()
            conn7.conn.close()
        ...
        if len(pers) == 0:  #推广人为空
            ...
            conn8.conn.cursor().execute(sql4,(t1,user))
            conn8.conn.commit()
            conn8.conn.close()
            ...
            t2=isovertime[0]+datetime.timedelta(days=1)
            conn9.conn.cursor().execute(sql4,(t1,user))
            conn9.conn.cursor().execute(sql4,(t2,pers))
            conn9.conn.commit()
            conn9.conn.close()
